...
You are given a working chat application where users can send messages to each other. The code currently works, but it has several design issues that make it difficult to maintain, extend, and add new features like group chats or message filtering.
...
1. **Tight Coupling**: Users directly reference each other, creating tight coupling
2. **Complex Communication**: Adding new users requires modifying existing user classes
3. **No Central Control**: No central place to manage communication rules
4. **Hard to Extend**: Adding features like group chats or message filtering is difficult
5. **Violation of Single Responsibility**: Users handle both their own logic and communication logic
